...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # print("sample stats", sample_mean, sample_var)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        # bn_param.set('running_mean', running_mean)
        # bn_param.set('running_var', running_var)
        
        sx = (x-sample_mean)/(sample_var+eps)
        out = beta + gamma*sx
        
        cache = (x, sample_mean, sample_var, gamma, beta, bn_param)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    # print("bk", sample_mean, sample_var)
    mode = bn_param['mode']
    eps = bn_param.get('eps', 1e-5)
    sx = (x-sample_mean)/(sample_var+eps)
    ...
    # print(gamma)
    dx = np.zeros_like(x)
    # dmean = 1/(sample_var+eps)*(-1/n)
    # dstd = (x-sample_mean)*(-1)/(sample_var+eps)**2*\
    #     (1/2)*(1/sample_var)*(1/n)*2*(x-sample_mean)
    
    for i in range(x.shape[0]):
        for j in range(x.shape[1]):
            cdx = (-1/n)/(sample_var[j]+eps)+\
                (x[:,j]-sample_mean[j])*(-1)/(sample_var[j]+eps)**2\
                *(1/2)*(1/sample_var[j])*(1/n)*2*(x[i,j]-sample_mean[j])
            cdx[i] = (1-1/n)/(sample_var[j]+eps)+\
                (x[i,j]-sample_mean[j])*(-1)/(sample_var[j]+eps)**2\
                *(1/2)*(1/sample_var[j])*(1/n)*2*(x[i,j]-sample_mean[j])
            dx[i,j] = cdx.dot(dout[:,j])
            
    '''
    dx = (-1/n)/(sample_var+eps)+\
                (x-sample_mean)*(-1)/(sample_var+eps)**2\
                *(1/2)*(1/sample_var)*(1/n)*2*(x-sample_mean)
    # sdx = (1-1/n)/(sample_var+eps)+\
    #             (x-sample_mean)*(-1)/(sample_var+eps)**2\
    #             *(1/2)*(1/sample_var)*(1/n)*2*(x-sample_mean)
            # print(cdx)
    # dx[i] = sdx[i]
    ...
    '''
    # print(dx)
    # print("dx dout shape", dx.shape, dout.shape)
    # print(dx)
    dx *= gamma
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = (np.random.uniform(size=x.shape) < p).astype(np.float32)
        out = x*mask/p
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
        ...
        for j in range(F):
            ...
            for k in range(Hp):
                for l in range(Wp):
                    rf = image[:,stride*k:stride*k+HH,stride*l:stride*l+WW]
                    # print(rf.shape, kernel.shape)
                    # dw[j,:,:,:] += rf
                    # db[j] += 1
                    out[i,j,k,l] = np.sum(rf*kernel) + bias
        ...
        for j in range(F):
            ...
            for k in range(Hp):
                for l in range(Wp):
                    rf = image[:,stride*k:stride*k+HH,stride*l:stride*l+WW]
                    # print(rf.shape, kernel.shape)
                    dw[j,:,:,:] += rf*dout[i,j,k,l]
                    db[j] += 1*dout[i,j,k,l]
                    
                    dxp[i,:,stride*k:stride*k+HH,stride*l:stride*l+WW] += kernel*dout[i,j,k,l]
                    # out[i,j,k,l] = np.sum(rf*kernel) + bias
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for j in range(C):
            for k in range(oh):
                for l in range(ow):
                    ...
                    # print(x[i,j,stride*k:stride*k+ph,stride*l:stride*l+pw].shape,
                    #       np.argmax(x[i,j,stride*k:stride*k+ph,stride*l:stride*l+pw]))
                    idx = np.argmax(x[i,j,stride*k:stride*k+ph,stride*l:stride*l+pw])
        ...
        for j in range(C):
            for k in range(oh):
                for l in range(ow):
                    ...
                    #       np.argmax(x[i,j,stride*k:stride*k+ph,stride*l:stride*l+pw]))
                    idx = np.argmax(x[i,j,stride*k:stride*k+ph,stride*l:stride*l+pw])
                    dx[i,j,stride*k+idx//ph,stride*l+idx%ph] += dout[i,j,k,l]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    NN = N*H*W
    DD = C
    dout = np.einsum("ijkl->iklj", dout)
    dout = dout.reshape((NN,DD))
